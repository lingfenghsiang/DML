...
dml::test::job_t::job_t(const dml_path_t path) {
  ...
  if (DML_STATUS_OK != get_job_size_status) {
    ...
    auto init_job_status = dml_init_job(path, actual_job_ptr);

    if (DML_STATUS_OK != init_job_status) {
      m_status = init_job_status;

      std::string message =
          "System error: Hardware initialization failed!\n \t Status code: " +
...
dml::test::job_t::job_t(job_t&& other_job) noexcept : m_status(DML_STATUS_OK) {
...
auto dml::test::job_t::operator==(const dml::test::reference_job_t& ref_job)
    const noexcept -> ::testing::AssertionResult {
  const auto actual_job_ptr = reinterpret_cast<dml_job_t*>(m_job_ptr.get());
  auto compare_result = ::testing::AssertionSuccess();

  if ((DML_OP_BATCH == actual_job_ptr->operation) &&
      (!ref_job.destination_first.empty())) {
    uint32_t index = 0;
    while (true) {
    ...
    if (!ref_job.destination_first.empty()) {
      for (auto i = 0u; i < actual_job_ptr->destination_length; ++i) {
        if (actual_job_ptr->destination_first_ptr[i] !=
            ref_job.destination_first[i]) {
          std::cout << std::endl
                    << "destination_first fields are not equal:" << std::endl;

          std::cout << std::endl
                    << "library_job->destination_first_ptr[" << i << "]"
                    << " = "
                    << static_cast<uint32_t>(
                           actual_job_ptr->destination_first_ptr[i])
                    << std::endl
    ...
    if (!ref_job.destination_second.empty()) {
      for (auto i = 0u; i < actual_job_ptr->destination_length; ++i) {
        if (actual_job_ptr->destination_second_ptr[i] !=
            ref_job.destination_second[i]) {
          std::cout << std::endl
                    << "destination_second fields are not equal:" << std::endl;

          std::cout << std::endl
                    << "library_job->destination_second_ptr[" << i << "]"
                    << " = "
                    << static_cast<uint32_t>(
                           actual_job_ptr->destination_second_ptr[i])
                    << std::endl
    ...
    if (nullptr != ref_job.crc_checksum_ptr) {
      if ((*actual_job_ptr->crc_checksum_ptr) != (*ref_job.crc_checksum_ptr)) {
        std::cout << std::endl
                  << "crc_checksum fields are not equal" << std::endl;
    ...
    switch (actual_job_ptr->operation) {
      ...
      case DML_OP_DELTA_CREATE:
        if ((actual_job_ptr->offset & OWN_MULTIPLY_8_MASK) !=
            (ref_job.offset & OWN_MULTIPLY_8_MASK)) {
          std::cout << std::endl
                    << "offset is not in the pattern range" << std::endl;
    ...
    if (actual_job_ptr->result != ref_job.result) {
      ...
      std::cout << std::endl
                ...
                << " = " << static_cast<uint32_t>(ref_job.result) << std::endl
                << std::endl;
